...
public class ReportGenerator_rerun {

    private static final Logger logger = LogManager.getLogger(ReportGenerator_rerun.class);
    ...
    public static void main(String[] args) throws IOException {
        ...
        try {
            ...
            reportBuilder.setReportDirectory("target-rerun/");
            reportBuilder.setReportFileName(String.format("run_%s_rerun_splashlearn_web_results_%s", runId, now));
            reportBuilder.setReportTitle("SplashLearn Web Automation Results - Rerun");
            ...
            ReportGenerator.failedTestScenarioIDs = ReportGenerator.getJSONObject("target-rerun/result.json", "FAILED_TESTS_INFO");
...
//                System.out.println("Failed TC: " + entry.getKey() + " - " + entry.getValue());
            ...
            configuration.add(new File("target-rerun/cucumber-report/"));
            ...
            String link = new S3Helper().upload("target-rerun/" + reportBuilder.getReportFileName() + ".html");

            if (link == null)
                return;

            try (FileOutputStream o = new FileOutputStream("target-rerun/result.txt")) {
    ...
    private static void storeInWorkspace() {
        ...
        try (FileWriter file = new FileWriter("target-rerun/result.json")) {
