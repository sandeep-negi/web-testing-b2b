@b2c  @regression @b2c_final @2nd_child_Addition
Feature: Second child addition
  ...
  Scenario: Verify paid user is able to add  2nd child
    ...
    And the user performs below to add child:
      | page            | field      | action              | value                       | timeout |
      | menu_page       | add_player | waitForNoElement    |                             | 10      |
      | menu_page       | add_player | click    |                             |       |
      |                 |            | waitForUrlToContain | /parent-dashboard#/settings | 10      |
      | grade_selection | grade_two  | waitForElement      |                             |         |
      | grade_selection | grade_two  | click               |                             | 10      |
    And call custom method for purchasing math Quarterly plan for 2nd child using:
      | klass         | method    | params                     |
      | ParentLibrary | purchase1 | math, Quarterly, one_child |
    And call custom method for handling thankyou page by using:
      | klass         | method             |
      | ParentLibrary | handleThankYouPage |
    And the user performs below to add 2nd child details:
      | page                     | field                | action              | value                                 | timeout |
      |                          |                      | waitForUrlToContain | /student-form/add/src=home?firstchild | 10      |
      | edit_child_details_pages | child_name           | waitForElement      |                                       |         |
      | edit_child_details_pages | child_name           | clear               |                                       |         |
      | edit_child_details_pages | child_name           | type                | ${firstName}                          |         |
      | edit_child_details_pages | child_avatar         | click               |                                       |         |
      | edit_child_details_pages | grade_for_math       | click               |                                       |         |
      | edit_child_details_pages | learning_path_topic  | click               |                                       |         |
      | edit_child_details_pages | multiple_attempts    | click               |                                       |         |
      | edit_child_details_pages | change_reading_grade | click               |                                       |         |
